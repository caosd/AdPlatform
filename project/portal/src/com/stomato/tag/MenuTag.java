...
public class MenuTag extends BodyTagSupport {
	...
	public int doEndTag() throws JspException {
		...
		try {
			MenuService menuService = (MenuService) SpringContextUtil
					.getBean("menuService");
			...
			for (int i = 0, j = menuList.size(); i < j; i++) {
				...
				sb.append("<span class=\"icon-box\"><i class=\"icon-file-alt\"></i></span>"
						+ menu.getName());
				sb.append("<span class=\"arrow\"></span>");
				sb.append("</a>");
				sb.append("<ul class=\"sub\">");
				for (Menu sunMenu : menu.getSunMenu()) {
					sb.append("<li><a class=\"\" href=\"" + sunMenu.getPath()
							+ "\">" + sunMenu.getName() + "</a></li>");
				}
				sb.append("</ul>");
				sb.append("</li>");
			}
