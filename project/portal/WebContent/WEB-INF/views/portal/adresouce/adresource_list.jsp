							...
							<tbody>
								<c:forEach items="${adResourceList}" var="item" varStatus="stat">
					            		<tr class="gradeX ${(stat.index%2) == 0 ? 'odd':'even' }">
                                            <td>${item.id}</td>
					                        <td><img width="50" src="${imgServer}${dir}${item.adIcon}"/></td>
					                        <td>
					                        ...
					                        <td class=" ">
					                        	<a href="/adresource/${item.id }/edit.html">编辑</a>
					                        	<c:if test="${item.status==1}">
					                        		<a href="/adresource/${item.id }/offshelf.html">下架</a>
					                        	</c:if>
					                        	<c:if test="${item.status==0}">
					                        		<a href="/adresource/${item.id }/onshelf.html">上架</a>
					                        	</c:if>
					                        	<c:choose>
					                        		<c:when test="${item.isRecom}">
					                        			<a href="/adresource/${item.id }/cancelrecomm.html">取消推荐</a>	
													</c:when>
					                        		<c:otherwise>
														<a href="/adresource/${item.id }/recomm.html">推荐</a>
													</c:otherwise>
					                        	</c:choose>
					                        	<a href="javascript:void(0)" onclick="deleteData('/adresource/${item.id }/delete.html')">删除</a>
					                        </td>
					                    </tr>
					           </c:forEach>
							</tbody>
