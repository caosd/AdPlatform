define(function(require) {
  ...
  function Popover(content_template, cls) {
    this.el = $('<div class="'+(cls || 'js-popover')+'"></div>').hide().appendTo('body').css('position', 'absolute')
  ...
  proto.open = function(attach_to_el, options) {
    options = $.extend({
      rerender: true
    , nib: true
    ...
    function rendered() {
      ...
      self.el
          .css({'left': ~~(bound_0[0]-attach_to_middle)+'px', 'top': ~~(bound_1[1]+5)+'px'})
          .show()
    }
  }
