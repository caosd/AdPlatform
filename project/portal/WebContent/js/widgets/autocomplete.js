define(function(require) {
  ...
  proto.base_template = [
      '{% for item in items %}'
    , '<li>{{ item }}<a href="#" class="btn mini primary" rel="item">Select</a></li>'
    , '{% endfor %}'
  ].join('')
  ...
  proto.keydown = function(ev) {
    ...
    function load(key) {
      ...
      var rex = /<(.*?)>; rel="(.*?)"/
        , link = self.headers.link.slice()
        , bits = {}
        , match

      while(match = rex.exec(link)) {
        bits[match[2]] = match[1]
        link = link.slice(match[0].length)
      }

      if(bits[key]) {
        self.el.addClass('loading')
        self.url_template_stream.emit('data', bits[key])
      }
    }
  }
